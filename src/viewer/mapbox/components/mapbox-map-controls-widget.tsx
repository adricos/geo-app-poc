...
export function MapboxMapControlsWidget() {
  ...
  const cycleViewState = useCallback(() => {
    setViewState((prev) => (prev === 'icon' ? 'styles' : prev === 'styles' ? 'full' : 'icon'));
  }, []);
  ...
  useEffect(() => {
    ...
    const onStyleData = () => { if (map.getStyle().layers?.length) refreshLayers(); };
  ...
  const setLayerVisibility = useCallback(
    (layerId: string, visible: boolean) => {
      ...
      try {
        ...
        setLayers((prev) =>
          prev.map((l) => (l.id === layerId ? { ...l, layout: { ...l.layout, visibility: visible ? 'visible' : 'none' } } : l)),
        );
      } catch {
  ...
  const handleStyleChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      setMapStyleKeyMapbox(e.target.value as MapboxStylePresetKey);
    },
    [setMapStyleKeyMapbox],
  );
